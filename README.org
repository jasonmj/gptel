...
- *NEW*: =gptel-workflow= provides a standalone workflow dispatcher for orchestrating chained, validated LLM workflows (plan → diff → tests → review → checklist) with acceptance criteria tracking, validation gates, and transient UI.  See [[file:gptel-workflow-README.md][gptel-workflow documentation]].
